    ...
    return(
       ...
       <li className="nav-item dropdown mr-3">
            ...
            <div className="dropdown-menu dropdown-menu-right">
                {props.dropdownItems.map((element, i) => {
        ...
        <li className="nav-item dropdown mr-3">
             ...
             <div className="dropdown-menu dropdown-menu-right">
                 {props.dropdownItems.map((element, i) => {
                     return (<a  className={"dropdown-item " + element.state} href={element.href} key={element.key} onClick={(e) => {e.preventDefault(); element.onClick()}}><i className="material-icons align-middle mr-1">message</i> {element.text}</a>)
                 })}
             </div>
         </li>
         )
     }else if (props.type === "link badge"){
        return (
            <li className="nav-item mr-3">
            <a className={"nav-link color-0A9FF2 " + props.state }href={props.href} data-toggle={props.dataToggle} onClick={(e) => {e.preventDefault(); props.onClick()}} data-target={props.dataTarget}><span style={{position:"relative"}}><i className="material-icons align-middle mr-1">{props.icon}</i><span className="proposals-list">{props.count}</span></span> {props.text}</a>
        </li>
        )
     }
}